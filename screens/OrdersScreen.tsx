...
import { View, Text, ScrollView, TouchableOpacity, TextInput, Alert, ActivityIndicator, Image } from 'react-native';import { Ionicons } from '@expo/vector-icons';
         ...
         return (
           <View key={step} style={{ flex: 1, alignItems: 'center' }}>
             <View style={{ flexDirection: 'row', alignItems: 'center', width: '100%' }}>
               {i > 0 && <View style={{ flex: 1, height: 3, backgroundColor: i <= currentIndex ? '#FF4500' : '#eee' }} />}
               <View style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: done ? '#FF4500' : '#eee', justifyContent: 'center', alignItems: 'center' }}>
                 <Text style={{ fontSize: 14 }}>{done ? info.emoji : '○'}</Text>
               </View>
               {i < STATUS_STEPS.length - 1 && <View style={{ flex: 1, height: 3, backgroundColor: i < currentIndex ? '#FF4500' : '#eee' }} />}
             </View>
             <Text style={{ fontSize: 9, color: done ? '#FF4500' : '#999', marginTop: 4, textAlign: 'center' }}>{info.label}</Text>
           </View>
         );
       })}
     </View>
    ...
    <View style={{ backgroundColor: '#fff', borderRadius: 20, marginBottom: 14, overflow: 'hidden', elevation: 3, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 10 }}>
      ...
      <View style={{ padding: 16 }}>
        ...
        {status === 'sent' && order.courierPhone && (
          <View style={{ backgroundColor: '#F3E5F5', borderRadius: 12, padding: 12, marginTop: 8, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <Ionicons name="call" size={18} color="#9C27B0" />
            <View>
              <Text style={{ color: '#9C27B0', fontWeight: '700', fontSize: 14 }}>Телефон курьера</Text>
        ...
        {/* Ожидает подтверждения оплаты Kaspi */}
    ...
    <View style={{ backgroundColor: '#fff', borderRadius: 20, marginBottom: 14, overflow: 'hidden', elevation: 3, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 10 }}>
      ...
      {expanded && (
        <View style={{ paddingHorizontal: 16, paddingBottom: 16 }}>
          ...
          {nextStatus === 'sent' && (
            <View style={{ marginBottom: 14 }}>
              <Text style={{ fontWeight: '700', color: '#1a1a1a', fontSize: 13, marginBottom: 8 }}>📱 Телефон курьера</Text>
              <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#f8f8f8', borderRadius: 12, paddingHorizontal: 14, borderWidth: 1.5, borderColor: '#eee' }}>
          ...
          {nextStatus ? (
            loading ? <ActivityIndicator color="#FF4500" /> : (
              <TouchableOpacity
                style={{ backgroundColor: '#FF4500', borderRadius: 14, padding: 15, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8, shadowColor: '#FF4500', shadowOpacity: 0.3, shadowRadius: 8, elevation: 3 }}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
      ...
      <View style={{ flexDirection: 'row', gap: 10, padding: 16, backgroundColor: '#fff', elevation: 2 }}>
        <TouchableOpacity
          style={{ flex: 1, paddingVertical: 10, borderRadius: 12, backgroundColor: filter === 'active' ? '#FF4500' : '#f5f5f5', alignItems: 'center' }}
          onPress={() => setFilter('active')}
        >
          <Text style={{ color: filter === 'active' ? '#fff' : '#666', fontWeight: '700' }}>
            🔔 Активные ({activeOrders.length})
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={{ flex: 1, paddingVertical: 10, borderRadius: 12, backgroundColor: filter === 'all' ? '#FF4500' : '#f5f5f5', alignItems: 'center' }}
          onPress={() => setFilter('all')}
        >
          <Text style={{ color: filter === 'all' ? '#fff' : '#666', fontWeight: '700' }}>
            📋 Все ({orders.length})
          </Text>
        </TouchableOpacity>
      </View>
        ...
        {displayed.length === 0 ? (
          <View style={{ alignItems: 'center', marginTop: 60 }}>
            ...
            <Text style={{ color: '#999', textAlign: 'center', fontSize: 14 }}>
              Найдите скидки и сделайте первый заказ!
            </Text>
          </View>
